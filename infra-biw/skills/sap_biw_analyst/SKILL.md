---
name: sap_biw_analyst
description: >-
  Conocimiento de dominio completo: queries KH Lloreda, dimensiones, medidas,
  versiones, ejemplos de uso. CARGAR SIEMPRE.
metadata:
  category: data
  agent: sap_analyst
  display-name: "SAP BIW Analyst"
---
...
### Queries disponibles (whitelist del proxy = fuente de verdad)

- **Ejecutables ahora** = whitelist del proxy. Tiene un **número concreto** (`count` de `biw_listQueries`).
- Si preguntan **cuántas** puedes consultar: **OBLIGATORIO** llamar a `biw_listQueries` y responder:
  > “Puedo consultar **N** queries de SAP BIW (las que expone el proxy). Si necesitas alguna que no esté en la lista, un administrador debe añadirla a la whitelist del proxy.”
- **PROHIBIDO** decir que no hay límite, que no hay número cerrado, o inventar “13”.
- **Alta de queries nuevas**: es tarea de administración del proxy (día-2), fuera de tus herramientas. No puedes añadirlas tú.
- Las tablas siguientes son **guía de uso preferido** (no el inventario completo).

#### Ventas y Facturación (ZBOKCOPA) — guía preferida
...
| Query | Uso | Medidas | Notas |
|---|---|---|---|
| `ZCOPACAR/MKT_CUENTA_RESULT_C_P_OPT_PC` | **⭐ DEFAULT.** P&L marketing (Provisión Cargos) | VN, costes, márgenes; dimensión **EAN/UPC** (`0MATERIAL__0EANUPC`) | **Requiere `0VERSION: #`**. Query por defecto del agente |
| `ZBOKCOPA/MKT_CUENTA_RES_CP_OPT_DPCT` | P&L mensual (vista marketing Real/Obj Det PCT) | 78 (VN, costes, todos los márgenes) | Alternativa. **Requiere `0VERSION: #`**. Tiene VINTMES (auto-expandido) |
...
**Producto:**
- `0MATERIAL` - **Código material / referencia / SKU** (clave interna; PT empieza por 5). **NO es EAN**
- `0MATERIAL__0EANUPC` - **EAN/UPC** (código de barras de la botella). **Solo existe en** `ZCOPACAR/MKT_CUENTA_RESULT_C_P_OPT_PC`. Si piden EAN: esa query + esta dimensión. **NUNCA** `0MATERIAL`, **NUNCA** `POWER_BI_PYG`/`DPCT` (no tienen EAN/UPC).

### Orden de consulta
1. **Primero** `ZCOPACAR/MKT_CUENTA_RESULT_C_P_OPT_PC` (default: ventas, P&L, EAN…).
2. **Solo después** otras queries si la default no cubre el dato o el usuario pide más información / otra query por nombre.
...
**IMPORTANTE**: Queries con dimensión `0VERSION` (queries P&L: `MKT_CUENTA_RESULT_C_P_OPT_PC`, `MKT_CUENTA_RES_CP_OPT_DPCT`, `POWER_BI_PYG`, `MKT_CUENTA_RES_PBI`, `MKT_CUENTA_RES_PBI_ZMSCOPA`) **suman todas las versiones** si no filtras. Esto produce números inflados (ej: 163M en vez de 54M). **Siempre filtra `"0VERSION": "#"` al consultar datos reales de estas queries.**
...
- `MKT_CUENTA_RESULT_C_P_OPT_PC` → **DEFAULT** P&L (Provisión Cargos; tiene EAN/UPC)
- `MKT_CUENTA_RES_CP_OPT_DPCT` → P&L con variables de tiempo (alternativa)
...
- `biw_listCatalogs` — Lista catálogos disponibles (InfoCubes/MultiProviders) del InfoSite ZKH_LLOREDA
- `biw_listQueries` — **Whitelist del proxy** (queries ejecutables ahora; fuente de verdad del listado; filtrable por catálogo/texto)
- `biw_getQueryMetadata` — Obtiene metadata completa de una query: dimensiones y medidas con nombres y descripciones
- `biw_getDimensionValues` — Obtiene valores posibles (miembros) de una dimensión dentro de una query
- `biw_getQueryVariables` — Obtiene las variables SAP BW definidas en una query. Variables con procType=1 son customer exits. biw_executeQuery auto-maneja variables de intervalo y de valor obligatorio
- `biw_executeQuery` — Ejecuta query estructurada contra SAP BIW. Especifica query, medidas, dimensión de desglose y filtros. El servicio genera MDX internamente y auto-resuelve VARIABLES SAP
- `biw_executeMdx` — MDX directo (solo si biw_executeQuery no basta)
...
- **Análisis P&L completo** → `MKT_CUENTA_RESULT_C_P_OPT_PC` (default) o `POWER_BI_PYG` / `MKT_CUENTA_RES_CP_OPT_DPCT`
- **Rentabilidad por cliente** → `BO_RENT_CLIENTE`
- **Stock** → `PBI_ST_001`
- **Tesorería** → `ZTRMP00_Q0001` (siempre selecciona medidas específicas — 1584 en total)

Si no estás seguro, usa `biw_getQueryMetadata` en las queries candidatas para inspeccionar sus medidas.

### Paso 2: Obtener metadata si es necesario

Llama a `biw_getQueryMetadata` para descubrir dimensiones y medidas disponibles. Es esencial cuando:
...
1. `biw_executeQuery(query=Q002, dimension="ZSEGMEN", filters={"0CALMONTH": "202601"})`
2. `biw_executeQuery(query=Q002, dimension="0MATERIAL__YCOPAPH1", filters={"0CALMONTH": "202601"})`
...
6. **Una dimensión por llamada**: Cada `biw_executeQuery` desglosa por una sola dimensión. Para análisis multi-dimensional, combina varias llamadas.
...
```
Pensamiento: P&L → MKT_CUENTA_RESULT_C_P_OPT_PC (default). Esta query tiene dimensión 0VERSION,
